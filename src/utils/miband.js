...
const { Buffer, createCipheriv } = require('browser-crypto');
...
// This is a helper function that c{onstructs an ArrayBuffer based on arguments
const AB = function() {
  let args = [...arguments];
  ...
  _handleNotify(event) {
    ...
    if (event.target.uuid === this.char.auth.uuid) {
      ...
      if (cmd === '100101') {         // Set New Key OK
        ...
        let cipher = createCipheriv('aes-128-ecb', this.key, '').setAutoPadding(false)
